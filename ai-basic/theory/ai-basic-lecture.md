...
- [RAG의 모든 것](#RAG의-모든-것)
  - [1. AI와 언어모델 기초 이해](#1-AI와-언어모델-기초-이해)
    ...
    - [1.7 Trasnformer란 무엇인가?](#17-Trasnformer란-무엇인가)
...
자연어 처리는 시대에 따라 크게 다른 접근 방식을 사용해왔습니다. 현재의 딥러닝 기반 NLP를 이해하기 위해서는 과거의 전통적 방법론을 먼저 이해하는 것이 중요합니다.

##### 1.2.1 전통적인 빈도수 기반 모델 (2010년대 이전)

2010년대 초반, 딥러닝 혁명이 일어나기 전까지 텍스트를 숫자로 표현하는 주요 방법론들은 단어의 빈도수와 통계적 중요도를 기반으로 했습니다.

**1) BoW (Bag-of-Words)**
- 문장에서 단어의 순서는 무시하고, 오직 각 단어의 출현 빈도만을 기록하여 벡터를 만듭니다
...
빈도수 기반 모델은 구현이 간단하고 계산 속도가 빠르다는 장점이 있었지만, 다음과 같은 치명적인 한계가 있었습니다.

| 한계점 | 설명 |
| --- | --- |
| **의미적 관계 파악 불가** | '고양이'와 '야옹이'는 의미가 유사하지만, 빈도수 벡터에서는 완전히 독립된 다른 단어로 처리됩니다. 단어의 유사성을 이해하지 못합니다. |
| **문맥 무시** | BoW는 단어의 순서를 무시하므로, "개가 사람을 물었다"와 "사람이 개를 물었다"를 거의 같은 문장으로 인식합니다. 순서가 중요한 문법적/의미적 차이를 잡아내지 못합니다. |
| **희소성 문제** | 전체 단어 수가 늘어 벡터의 차원이 기하급수적으로 커지고 대부분이 0으로 채워지는 희소 벡터(Sparse Vector) 문제가 발생하여 계산이 비효율적이었습니다. |

##### 1.2.3 딥러닝 시대의 시작 (2010년대 중반)

이러한 한계를 극복하기 위해 등장한 것이 **딥러닝 기반의 임베딩(Word Embedding)** 기술입니다.
...
2017년 **Transformer** 등장 이후, NLP는 완전히 새로운 차원으로 진화했습니다.
