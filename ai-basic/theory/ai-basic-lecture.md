...
- **단기 (1-2년)**
  - 트랜스포머가 여전히 주도적 위치
  - Mamba는 특정 도메인에서 점진적 도입
  - 하이브리드 접근법 (Transformer + Mamba) 등장
...
#### 1.10 지식 증류(Knowledge Distillation)

##### 지식 증류란?

- **정의**: 큰 모델(Teacher)의 지식을 작은 모델(Student)에 전달하는 기법
- **목적**: 성능은 유지하면서 모델 크기와 추론 속도를 대폭 개선
- **핵심**: Teacher의 "soft knowledge"를 Student가 학습

##### 지식 증류의 원리

- **Soft Target vs Hard Target**
  - **Hard Target**: `[0, 0, 1, 0, 0]` (정답만 1, 나머지 0)
  - **Soft Target**: `[0.1, 0.2, 0.6, 0.05, 0.05]` (확률 분포)
  - **장점**: Teacher의 확신도와 불확실성 정보 전달

- **Temperature Scaling**
  - Softmax 함수에 온도 파라미터 T 적용
  - T가 높을수록 분포가 부드러워짐
  - 예: `softmax(logits/T)`
...
- **장점**
  - 모델 크기와 추론 비용 대폭 감소
  - 배포 환경에서의 실용성 향상
  - Teacher의 지식을 효율적으로 전달

- **단점**
  - Teacher 모델이 필요 (추가 비용)
  - 복잡한 지식은 전달하기 어려움
  - 일부 태스크에서는 성능 손실 발생
